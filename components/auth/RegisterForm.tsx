...
export default function RegisterForm() {
  ...
  const getInputStyles = (fieldName: keyof RegisterFormValues) => {
    ...
    return cn(
      'w-full px-5 py-4 pl-12 bg-[#F4F3EE] border border-[#B1ADA1]/30 rounded-2xl outline-none transition-all text-stone-900',
      !isTouched && 'focus:ring-2 focus:ring-[var(--color-primary)] focus:bg-white',
      isTouched && hasError && 'ring-2 ring-red-400 focus:ring-red-500 bg-red-50/30 text-stone-900 placeholder:text-red-300',
      isTouched && !hasError && 'ring-2 ring-[#B1ADA1] focus:ring-[var(--color-primary)] bg-white text-stone-900 focus:bg-white',
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-md bg-white/90 backdrop-blur-xl rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] p-8 relative z-10 border border-[#B1ADA1]/20 my-4 text-stone-900"
    >
      ...
      {isSuccess && (
        ...
        >
          <div className="text-center mb-6">
            <div className="w-16 h-16 mx-auto bg-[#F4F3EE] rounded-2xl shadow-sm border border-[#B1ADA1]/30 flex items-center justify-center mb-6 text-[var(--color-primary)]">
              <Mail size={32} />
            </div>
            <h3 className="text-3xl font-bold text-stone-900 tracking-tight">Revisa tu correo</h3>
            <p className="text-sm text-stone-500 mt-3 px-4">
          ...
          <form onSubmit={onVerifySubmit} className="space-y-4">
            <div className="relative">
              <input
                ...
                placeholder="A5Z9P2"
                className="w-full px-5 py-5 bg-[#F4F3EE] border border-[#B1ADA1]/30 rounded-2xl outline-none transition-all text-stone-900 text-center text-3xl tracking-[0.5em] font-mono focus:ring-2 focus:ring-[var(--color-primary)] focus:bg-white uppercase placeholder:tracking-normal placeholder:font-sans placeholder:text-lg"
              />
            </div>
      ...
      {!isSuccess && (
        <>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              {watchPassword.length > 0 && (
                <div className="ml-2 mr-2 pt-1 pb-1">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <p className={cn("text-[10px] font-bold uppercase tracking-widest", strengthScore >= Math.floor((strengthLabels.length - 1) / 2) ? "text-[var(--color-primary)]" : "text-stone-500")}>
            ...
            <p className="text-center text-sm text-stone-500 mt-6 pb-2">
              ¿Ya tienes cuenta?{' '}
              <Link
                replace
                href="/login"
