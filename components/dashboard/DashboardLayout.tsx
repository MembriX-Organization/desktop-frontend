...
import { Building2, Users, Calendar, CreditCard, LogOut, Menu, X, Shield } from 'lucide-react';
...
const navItems = [
  ...
  { href: '/dashboard/staff', icon: Shield, label: 'Staff' },
...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--color-pampas)]">
  ...
  const UserCard = () => (
    <div className="bg-white/60 border border-[#B1ADA1]/20 rounded-xl p-3 shadow-sm backdrop-blur-sm mt-4">
  ...
  return (
    <div className="min-h-screen bg-[var(--color-pampas)]">
      {/* Mobile Header */}
      ...
      <div className="flex">
        {/* Sidebar Desktop */}
        <aside className="hidden lg:flex lg:flex-col lg:w-64 bg-[var(--color-pampas)] border-r border-[#B1ADA1]/30 min-h-screen sticky top-0 h-screen">
          ...
          <div className="p-4 border-t border-[#B1ADA1]/30">
            <button
              onClick={logout}
              className="flex items-center space-x-3 px-4 py-3 rounded-xl text-red-600/80 hover:bg-red-50 hover:text-red-700 transition-all w-full text-sm font-medium"
            >
